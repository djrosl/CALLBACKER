...
Class TelphinCall {
    ...
    // конструктор класса
    function __construct( $app_key, $app_secret, $server_name ) {
    ...
    private function gen_token() {
        $url = 'https://' . $this->API_SERVER . '/oauth/token.php';
        ...
        curl_setopt( $req, CURLOPT_USERAGENT, 'TelphinWebCall-UnifiedScript' );
    ...
    private function get_filename() {
        return Yii::getAlias('@runtime/calls').'/._ttmc_' . md5( $this->APP_KEY . $this->APP_SECRET );
    }
    ...
    ***************************/
    public function make_call( $extension, $source, $destination, $caller_id, $call_type = 'simple' ) {

        if ( ! in_array( $call_type, array( 'simple', 'callback' ) ) )
            $call_type = 'simple';
        ...
        $url = 'https://' . $this->API_SERVER . '/uapi/phoneCalls/@me/' . $call_type;
...
define( 'MY_APP_KEY', 'L_9wJ9LDAT.S4PRPT3g~mjXCDjgq-4rX' );
define( 'MY_APP_SECRET', 'A-J2Mpr9Lv_MLjWsvOh-Onf87SWs_JXg' );
define( 'EXTERNAL_FIRST', false );
define( 'EXTENSION', '12005*101' );
define( 'LOCAL_NUMBER', '102' );
define( 'SERVER_NAME', "cloud.telphin.ru" );
...
function call_proccessing( $phonenumber ) {
    $tel_call = new TelphinCall( MY_APP_KEY, MY_APP_SECRET, SERVER_NAME );
    $source;
    $destination;
    if ( EXTERNAL_FIRST ) {
        $source = $phonenumber;
    ...
    $caller_id = "Web Call <$phonenumber>";
    return $tel_call->make_call( EXTENSION, $source, $destination, $caller_id, 'simple' );
}
        ...
        if ( $_SERVER['REQUEST_METHOD'] != 'POST' )
            ...
            if ( isset( $phonenumber ) ) {
                ...
                if ( isset( $call_res['error'] ) )
                    ...
                    $success_data['msg'] = 'Ожидайте звонка на введенный номер.';
                    return $success_data;
                }
            }
            else {
